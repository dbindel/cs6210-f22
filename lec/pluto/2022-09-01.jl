...
# ╔═╡ ecaaeeb3-f573-473b-be95-912a12ebc380
md"""
## Testing summation algorithms

In the notes, we described both a technique for computing a running error bound of a sum and a compensated summation technique for higher-precision evaluation of a sum.  Julia provides an arbitrary precision `BigFloat` package, and we can use this to evaluate both the bound and the compensated summation technique.
"""

# ╔═╡ 9c79c459-20b3-48b1-b6cc-9690047408ab
# Sum into a BigFloat accumulator for extra precision
function mysum_bf(v)
	s = BigFloat(0.0)
	for x in v
		s += x
...
# ╔═╡ 0ea7074d-0c9c-4556-be01-65fe2cb27d7f
# Ordinary sum with a running error bound
function mysum_running(v)
	s = 0.0
	e = 0.0
	for x in v
		s += x
...
# ╔═╡ 19da4a55-e10f-4e6b-951c-f15c4fbaa4b9
...
# ╔═╡ 81a2dfbc-188a-4b6e-94ab-7cee92395b18
let
	v = sort(randn(1000000))
	s2 = Float64(mysum_bf(v))
	s3, e3 = mysum_running(v)
	s4 = mysum_cs(v)

	relerr1 = abs((s3-s2)/s2)
	relerr2 = abs(e3/s2) 
	relerr3 = abs((s4-s2)/s2)

md"""
- Relerr (basic):  $relerr1
- Estimated bound: $relerr2
- Relerr (kahan):  $relerr3
"""
end
